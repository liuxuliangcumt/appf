...
class _DongGanDanCheMatchRoomPage extends State<DongGanDanCheMatchRoomPage> {
  ...
  Widget build(BuildContext context) {
    ...
    var matchPeopleList = matchInfo["matchPeopleList"] == null ? [] : matchInfo["matchPeopleList"];
    ...
    arrWidget.add(Expanded(
      ...
      child: SingleChildScrollView(
        child: Container(
          child: Column(
            children: _buildMatchPeopleList(matchPeopleList, boxWidth, boxHeight),
          ),
        ),
      ),
    ));
  ...
  Widget buildButton(bIsReady) {
    return Container(
      ...
      child: RawMaterialButton(
        ...
        onPressed: () {
          HttpUtil.getInstance().postJson(bIsReady ? API.beginMatch : API.endMatch, params: {
  ...
  void initData() {
    ...
    var callback = (timer) {
      ...
      if (matchType == '2' && mileage > totalMileage) {
        var matchId = widget.arguments["id"] == null ? 0 : widget.arguments["id"];
  ...
  void exit() {
    ...
    if (status != 3 && id > 0) {
      HttpUtil.getInstance().postJson(API.leaveMatchRoom, params: {
        "matchId": id,
        "userId": userId
      }).then((response) {});
    }
