...
class MatchListPage extends StatefulWidget {
  ...
  final needInit;
  MatchListPage(this.type, this.userId, this.refreshController, { this.needInit = true });
...
class _MatchListPage extends State<MatchListPage> {
  ...
  Widget buildMatchList(context) {
    ...
    matchData.asMap().keys.forEach((index) {
      ...
      var strImage = item['image'] == null || item['image'].length < 10 ? BaseUtil.gDefaultImage : BaseUtil.gBaseUrl + item['image'];
      var startTime = item['startTime'] == null ? "2021-10-01 10:10:10" : item['startTime'];
      ...
      switch (nMaxPeople) {
        case 2: { strPeopleTypeName = gArrMatchPeopleType[0]; } break;
        case 3: { strPeopleTypeName = gArrMatchPeopleType[1]; } break;
        case 6: { strPeopleTypeName = gArrMatchPeopleType[2]; } break;
        default: { strPeopleTypeName = gArrMatchPeopleType[3]; } break;
      }
      var status = item["status"] == null ? '0' : item["status"];
      /// 学员比赛未开始，显示提示报名
      var showTip = BaseUtil.gStUser["userType"] == 102 && status == '0' && DateTime.parse(startTime).difference(DateTime.now()).inSeconds > 0;
      matchList.add(
        GestureDetector(
          onTap: () {
            ...
            child: ClipRRect(
                ...
                child: Container(
                  ...
                  child: Row(
                    children: [
                      Container(
                        padding: EdgeInsets.fromLTRB(BaseUtil.dp(14), BaseUtil.dp(16), BaseUtil.dp(14), BaseUtil.dp(16)),
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Container(
                              constraints: BoxConstraints(maxWidth: BaseUtil.dp(150)),
                              child: Text(
                                name,
                                style: TextStyle(
                                    color: Colors.black,
