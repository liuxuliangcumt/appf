...
class DongGanDanCheMatchDetailPage extends StatefulWidget {
  ...
  @override
  _DongGanDanCheMatchDetailPage createState() => _DongGanDanCheMatchDetailPage();
}

class _DongGanDanCheMatchDetailPage extends State<DongGanDanCheMatchDetailPage> {
  ...
  Widget build(BuildContext context) {
    String title = '比赛详情';
    var strImage = widget.arguments['image'] == null || widget.arguments['image'].length < 10 ? BaseUtil.gDefaultImage : BaseUtil.gBaseUrl + widget.arguments['image'];
    var startTime = widget.arguments['startTime'] == null ? "2021-10-01 10:10:10" : widget.arguments['startTime'];
    startTime = DateFormat("yyyy-MM-dd HH:mm").format(DateTime.parse(startTime));
    var strName = widget.arguments['name'] == null ? "" : widget.arguments['name'];
    var matchType = widget.arguments["type"] == null ? 1 : int.parse(widget.arguments["type"]);
    var strTypeName = gArrMatchType[matchType - 1];
    var nMaxPeople = widget.arguments["maxPeople"] == null ? 2 : widget.arguments["maxPeople"];
    var nActual = widget.arguments["actual"] == null ? 0 : widget.arguments["actual"];
    var strRule = widget.arguments["rule"] == null ? "无" : widget.arguments["rule"];
    var fixData = widget.arguments["fixData"] == null ? 0.0 : widget.arguments["fixData"];
    var status = widget.arguments["status"];
    var bValidTime = DateTime.parse(startTime).difference(DateTime.now()).inSeconds > 0;
    ...
    if (bIsCreator && bValidTime && status == '0') {
      arrHeader.add(Container(
        ...
        padding: EdgeInsets.only(bottom: BaseUtil.dp(16), right: BaseUtil.dp(20)),
        child: Container(
              ...
              Container(
                ...
                child: Column(
                  children: [
                    ...
                    Padding(padding: EdgeInsets.only(top: BaseUtil.dp(12)),),
                    Row(
                      children: [
                        Container(
                        ...
                        Container(
                          ...
                          child: Text(
                            matchType == 1 ? "比赛时长 $fixData分钟" : "比赛里程 $fixData公里",
                            style: TextStyle(
                              color: Color(0xFF333333),
                              fontSize: BaseUtil.dp(14),
                            ),
    ...
    if (status == '0') {
      if (bValidTime) {
        if (BaseUtil.gStUser["userType"] == 102) {
          arrWidget.add(Container(
            padding: EdgeInsets.symmetric(vertical: BaseUtil.dp(10), horizontal: BaseUtil.dp(20)),
          ...
          arrWidget.add(Container(
            ...
            child: RawMaterialButton (
              ...
              onPressed: createMatchRoom,  /// 点击开启比赛房间
              child: Center(
                child: Text(
                  '开启比赛房间',
                  style: TextStyle(
      ...
      if (bIsCreator || (BaseUtil.gStUser["userType"] == 102 && bIsJoined)) {
        arrWidget.add(Container(
          ...
          child: RawMaterialButton (
            ...
            onPressed: enterMatchRoom,  /// 点击进入比赛房间,准备比赛
            child: Center(
              child: Text(
                '进入比赛房间',
                style: TextStyle(
      ...
      arrWidget.add(Container(
        ...
        child: RawMaterialButton (
          ...
          onPressed: watchResult,  /// 点击查看比赛成绩
          child: Center(
            child: Text(
              '查看成绩',
              style: TextStyle(
  ...
  /// 进入比赛房间
  void enterMatchRoom() {
    var bicycleLogId = BaseUtil.gStBicycleLog != null && BaseUtil.gStBicycleLog["id"] != null ? BaseUtil.gStBicycleLog["id"] : 0;
    /// 学员必须先连接设备，然后进入房间
