...
abstract class API {
  ...
  static const batchUpdateBicycleDrag = '/app/batchUpdateBicycleDrag'; // 获取系统配置信息
...
class HttpUtil {
  ...
  HttpUtil() {
    ...
    InterceptorSendCallback cbRequest = (RequestOptions options, RequestInterceptorHandler handler) {
      print("\n================== 请求数据 ==========================");
    ...
    InterceptorSuccessCallback cbResponse = (Response response, ResponseInterceptorHandler handler) {
      print("\n================== 响应数据 ==========================");
    ...
    InterceptorErrorCallback cbError = (DioError e, ErrorInterceptorHandler handler) {
      print("\n================== 错误响应数据 ======================");
      print("type = ${e.type}");
    ...
    // 添加拦截器
    mDio.interceptors.add(InterceptorsWrapper(onRequest: cbRequest, onResponse: cbResponse, onError: cbError));
  }
  ...
  Future<Response> put(String url, { dynamic params, Options option, bool hasToken = true }) async {
    if(option == null) {
      option = Options();
    }
    option.method = "PUT";
  ...
  Future<Response> get(String url, { dynamic params, Options option, bool hasToken = true }) async {
    if(option == null) {
      option = Options();
    }
    option.method = "GET";
  ...
  Future<Response> postForm(String url, { dynamic params, Options option, bool hasToken = true }) async {
    if(option == null) {
      option = Options();
    }
    option.method = "POST";
  ...
  Future<Response> postJson(String url, { dynamic params, Options option, bool hasToken = true }) async {
    if(option == null) {
      option = Options();
    }
    option.method = "POST";
  ...
  Future<Response> uploadFile(String strURL, String paramName, String strFilePath, { bool hasToken = true, ext = "" }) async {
    var strName = strFilePath.substring(strFilePath.lastIndexOf("/") + 1, strFilePath.length);
    if (ext != "") {
      strName = strName.substring(0, strName.lastIndexOf("."));
      strName = strName + "." + ext;
    }
    var postData = FormData.fromMap({paramName: await MultipartFile.fromFile(strFilePath, filename: strName)});//file是服务端接受的字段字段，如果接受字段不是这个需要修改
    var option = Options(
        method: "POST",
...
//              "%"); //取精度，如：56.45%
  ...
  Future<void> getUserInfo(context) async {
    HttpUtil.getInstance().get(API.getUserInfo, params: {}).then((res) {
      ...
      Navigator.of(context).pushNamedAndRemoveUntil("/index", (Route<dynamic> route) => false, arguments: 0);
    });
  }
